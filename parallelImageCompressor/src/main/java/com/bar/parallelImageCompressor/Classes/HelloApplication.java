...
import com.bar.parallelImageCompressor.Classes.Graph.Graph;
...
//    }
    ...
    public HelloApplication(BufferedImage image) {
        ...
        try {
            ImageIO.write(imageToConvert, "bmp", file);
        } catch (IOException e) {
    ...
    private BufferedImage lossyDisplay(File file) throws IOException {
        ...
        imageWidth = Math.abs((int)width);
        imageHeight = Math.abs((int)height);
    ...
    private void lossyDecode() {
        ...
        for (int i = 0; i < imageWidth; i++) {
            for (int j = 0; j < imageHeight; j++) {
                ...
                Color color = new Color(r, g, b);
                graph.setPixel(i, j, color.getRGB());
            }
        }
